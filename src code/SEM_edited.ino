...
#define AL_PIN 5 //change to pin 5 and wire to 20 //directly opposite on the board
#define BH_PIN 22
#define BL_PIN 9
#define CH_PIN 4 //change to pin 4 and wire to 21 //directly opposite on the board
#define CL_PIN 10

//pins 20 and 21, used in our pcb for teensy 3.2, no longer pwm compatible
//#define PIN_20 20
//#define PIN_21 21
...
void setup() {                // The setup function is called ONCE on boot-up
 ...
 //pinMode(PIN_21,INPUT); //Kozel said either set these to open drain or input                
                  

  analogWriteFrequency(AH_PIN, 8000); // Set the PWM frequency.
  analogWriteFrequency(AL_PIN, 8000);
  analogWriteFrequency(BH_PIN, 8000);
  analogWriteFrequency(BL_PIN, 8000);      
  analogWriteFrequency(CH_PIN, 8000);
  analogWriteFrequency(CL_PIN, 8000);
...
/* Read hall sensors WITH oversampling. This is required, as the hall sensor readings are often noisy.
